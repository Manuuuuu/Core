...
-> Collision
                 ...
                 -> Minion health prediction: VPrediction will check if the minions will be dead when the skillshot reaches their location. You can check how this works in this videos: http://botoflegends....-14#entry229428
...
Examples:
         ...
         -Line skillshot (Blitzcrank Q for example):
    ...
    function OnTick()
        for i, target in pairs(GetEnemyHeroes()) do
            local CastPosition,  HitChance,  Position = VP:GetLineCastPosition(target, 0.6, 75, 650, 1800, myHero, true)
            if HitChance >= 2 and GetDistance(CastPosition) < 850 then
                CastSpell(_Q, CastPosition.x, CastPosition.z)
            end
        end
    end
-Circular positions (Cassiopeia's Q for example):
    local VP = nil
    function OnLoad()
        VP = VPrediction()
    end
    function OnTick()
        for i, target in pairs(GetEnemyHeroes()) do
            local CastPosition,  HitChance,  Position = VP:GetCircularCastPosition(target, 0.6, 75, 650)
            if HitChance >= 2 and GetDistance(CastPosition) < 850 then
                CastSpell(_Q, CastPosition.x, CastPosition.z)
            end
        end
...
function VPrediction:__init()	
	...
	self.BlackList = { {name = "aatroxq", duration = 0.75}, } --[[4 Dashes, OnDash fails]]
...
--[[
	if VIP_USER then load(Base64Decode(""))() end
	]]
function VPrediction:GetTime() return os.clock() end
--[[Track when we lose or gain vision over an enemy]]
function VPrediction:OnGainVision(unit) if unit.type == myHero.type then self.TargetsVisible[unit.networkID] = self:GetTime() end end
--[[Track when we lose or gain vision over an enemy]]
function VPrediction:OnLoseVision(unit) if unit.type == myHero.type then self.TargetsVisible[unit.networkID] = math.huge end end
